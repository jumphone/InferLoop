...
SUB=c()
################################################################
real.ref=readRDS('coolCount_100k/COM_real.ref.rds')
real.ref=real.ref[which(rowSums(real.ref)>0),]
pred.ref=readRDS('./ics_out/COM_pred.ref_100k.rds')
this_out=.getSub(real.ref,pred.ref)
OUT[[1]]=this_out$yes
OUT[[2]]=this_out$no_head
################################################################
real.ref=readRDS('coolCount_200k/COM_real.ref.rds')
real.ref=real.ref[which(rowSums(real.ref)>0),]
pred.ref=readRDS('./ics_out/COM_pred.ref_200k.rds')
this_out=.getSub(real.ref,pred.ref)
OUT[[3]]=this_out$yes
OUT[[4]]=this_out$no_head
################################################################
real.ref=readRDS('coolCount_300k/COM_real.ref.rds')
real.ref=real.ref[which(rowSums(real.ref)>0),]
pred.ref=readRDS('./ics_out/COM_pred.ref_300k.rds')
this_out=.getSub(real.ref,pred.ref)
OUT[[5]]=this_out$yes
OUT[[6]]=this_out$no_head
################################################################
real.ref=readRDS('coolCount_400k/COM_real.ref.rds')
real.ref=real.ref[which(rowSums(real.ref)>0),]
pred.ref=readRDS('./ics_out/COM_pred.ref_400k.rds')
this_out=.getSub(real.ref,pred.ref)
OUT[[7]]=this_out$yes
...
pdf('f10p02_sub_barplot_cortex.pdf',width=2.5,height=5)

barplot(SUB,ylim=c(0,0.15),col=c(rep('grey60',0),rep('indianred1',4)),
