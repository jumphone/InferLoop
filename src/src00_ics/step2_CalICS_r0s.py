...
def calZ(a, b, r):
    ...
    C = np.sum(b_delta ** 2)
    ##########################
    a_delta_real = a - np.mean(a)
    b_delta_real = b - np.mean(b)
    A_real = np.sum(a_delta_real * b_delta_real)
    B_real = np.sum(a_delta_real ** 2)
    C_real = np.sum(b_delta_real ** 2)
    this_dist_real = A_real / np.sqrt( B_real * C_real )
    #########################
    this_dist = A / np.sqrt( B * C )
    this_dist_plus = (A + a_delta * b_delta) / np.sqrt( (B + a_delta**2) * (C + b_delta**2) ) 
    this_delta = this_dist_plus - this_dist
    ##########################
    this_std = (1-this_dist**2)/(n-1)  
    if this_std > 0: 
        this_z = this_delta / this_std
    else:
        this_z = this_delta
    ##########################
    this_z = this_z * (this_dist_real+1) 
    return(this_z)
